        ...
        for i in range(2, new_population_out.shape[2] - 2, 2):
            ...
            while c%2 == 1:
                ...
                for m in range(num_edge_out.shape[0]):
                    ...
                    #to test if the point is in the 
                    if c%2 == 1:
                        break
                    n2 += num_edge_out[m]
            if y == 0:
...
@cuda.jit('(float32[:, :, :], float32[:, :], float32[:, :], int32[:], float32[:, :], float32[:, :], float32[:, :], float32[:, :], int32[:], float32[:, :] ,float32[:, :], float32[:, :], float32[:, :], float32[:, :], float32[:, :], float32[:, :], int32[:], int32[:, :], float32[:, :], float32[:, :], float32[:, :], float32[:], float32[:], float32[:], float32[:])')
def fitness(pop, obstacles_out,  obstacles_m_all_out, num_edge_all_m_out, position_1_out, position_2_out, position_3_out, position_4_out, num_edge_out,obstacles_m_out, obstacles_m_2_out, position_2_1_out, position_2_2_out, position_2_3_out, position_2_4_out, obstacles_s_out, num_edge_s_out, intersection_value_out, length_out, smoothness_out, fitness_value_out, init_dir_out, dir_final_out, length_p_out, smoothness_p_out):
...
#######    ob_2    #######
        ...
        if intersection_value_out[x, y] == 0:
            ...
            for z in range(3):
                ...
                #if sum_dis != 0 and sum_dir != 0:
                #if  risk_det == 1:
                for i in range(num_edge_all_m_out.shape[0] - 1):
                    for j in range(num_edge_all_m_out[i]):
                        n1 = (j+1)%num_edge_all_m_out[i]
  
                        #coodinates of points C, D
                        C_x = obstacles_m_all_out[n+j, 0]
                        C_y = obstacles_m_all_out[n+j, 1]
                        D_x = obstacles_m_all_out[n+n1, 0]
                        D_y = obstacles_m_all_out[n+n1, 1]
        ...
        sum_risk = sum_risk/6
        fitness_value_out[x][y] = length/0.26 + smoothness/1.82  + 10000 * intersection_value_out[x, y] +  safety  + sum_risk  
