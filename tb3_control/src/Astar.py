...
@njit
def one_paths(dist, h, src, tar, num, points):
    path_feasible = []
    num_p = np.zeros(num).astype(np.int64)

    i = 0

    pa = astar(dist,h,src,tar)

    path_feasible.append(pa)
    path = np.zeros((num,int(2*len(pa)))).astype(np.float32)
    for i in range(num):
        n = 0;
        for j in range(len(path_feasible[0])):
            path[i,2*n] = points[path_feasible[0][len(path_feasible[0])-j-1],0]
            path[i,2*n+1] = points[path_feasible[0][len(path_feasible[0])-j-1],1]
            n += 1
...
@njit
def med_fitness_paths_L(dist, h, src, tar, num, points, L):
    path_feasible = []
    num_p = np.zeros(num).astype(np.int64)
    fitness_p = np.zeros(num).astype(np.float32)
    p = 0
...
@njit
def med_fitness_paths_show(dist, h, src, tar, num, points):
    path_feasible = []
    num_p = np.zeros(num).astype(np.int64)
    fitness_p = np.zeros(num).astype(np.float32)
    p = 0
    ...
    for i in range(len(path_t)):
        n = 0
        path_s = np.zeros(len(path_t[i])*2)
        for j in range(len(path_t[i])):
            path_s[2*n] = points[path_t[i][len(path_t[i])-j-1],0]
            path_s[2*n+1] = points[path_t[i][len(path_t[i])-j-1],1]
            n += 1
        path_total.append(path_s)
    path_f = []
    for i in range(len(path_feasible)):
        n = 0
        path_s = np.zeros(len(path_feasible[i])*2)
        for j in range(len(path_feasible[i])):
            path_s[2*n] = points[path_feasible[i][len(path_feasible[i])-j-1],0]
            path_s[2*n+1] = points[path_feasible[i][len(path_feasible[i])-j-1],1]
            n += 1
        path_f.append(path_s)
    return path, path_f, fitness_p, path_total
...
def fitness_path(path):
    smoothness = 0
    length = 0
    for i in range(0,path.shape[0]-2,2):
        if i < path.shape[0] - 4:
            dx_1 = path[i+2]-path[i]
            dy_1 = path[i+3]-path[i+1]
            dx_2 = path[i+4]-path[i+2]
            dy_2 = path[i+5]-path[i+3]
            direction = 180 * abs(math.atan2(dy_2*dx_1-dx_2*dy_1,dx_1*dx_2+dy_1*dy_2))/math.pi
            smoothness += direction
        A_x = path[i]
        A_y = path[i+1]
        B_x = path[i+2]
        B_y = path[i+3]
        length += math.sqrt((B_x-A_x)**2 + (B_y-A_y)**2)
    return length, smoothness
