import numpy as np
import torch
from generateScenarios.a_star import PathPlanner
...
class AgentState:
    ...
    def toInputTensor(self, goal_allagents, state_allagents, step, agent_path, R):
        ...
        for id_agent in range(self.num_agents):
            ...
            input_step_currentAgent, record_localPath = self.agentStateToTensor(load_state)
        ...
        feat_own_path, feat_agent_path, feat_final_goal = getState(1, self.num_agents, self.config.nearest_step_num, self.config.nearest_robot_num, agent_path, R, goal_allagents, step)

        agent_path_reshaped_pre = feat_agent_path[:, :, :, :].reshape((feat_agent_path.shape[0], feat_agent_path.shape[1], feat_agent_path.shape[2] * 2), order='F')
        ...
        own_path_reshaped = feat_own_path[:, :, :].reshape((feat_own_path.shape[0], feat_own_path.shape[1] * 2), order='F')
        final_goal_reshaped = feat_final_goal[:, :, :].reshape((feat_final_goal.shape[0], feat_final_goal.shape[1] * 2), order='F')

        final = np.hstack((own_path_reshaped, agent_path_reshaped))
        input_tensor_final = torch.FloatTensor(np.array(final))

        return input_tensor, input_tensor_final
    ...
    def agentStateToTensor_projectG(self, load_state):
