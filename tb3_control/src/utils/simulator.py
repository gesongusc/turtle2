...
import shutil
from policy.instance_generator import *
from policy.cbs import CBSSolver
...
class multiRobotSimNew:
    ...
    def __init__(self, config):
        ...
        for i in range(self.config.num_agents):
            self.forced.append(False)
            self.forced_path.append([])
    ...
    def getCurrentState(self, step):
        ...
        tensor_currentState, tensor_agentPath = self.AgentState.toInputTensor(store_goalAgents, store_stateAgents, step, self.path_list, (self.config.FOV - 1) / 2)
        tensor_currentState = tensor_currentState.unsqueeze(0)
        tensor_agentPath = tensor_agentPath.unsqueeze(0)

        agents_localPath = self.AgentState.get_localPath()
        self.store_localPath.append(agents_localPath)

        return tensor_currentState, tensor_agentPath
    ...
    def isNeighbour(self, a, b):
        if abs(a[0] - b[0]) + abs(a[1] - b[1]) == 0 or abs(a[0] - b[0]) + abs(a[1] - b[1]) == 1:
            return True
        else:
            return False

    def checkEqualPosition(self, a, b):
